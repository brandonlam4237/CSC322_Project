...
def getDict(url):
  ...
  # Get product name
  nameStep1 = driver.find_elements(By.XPATH, "//*[@id=\"product-details-control\"]/div[2]/h1/span")
  nameFinal = ""
  ...
  # Get brand name
  brandStep1 = driver.find_elements(By.XPATH, "//*[@id=\"product-details-control\"]/div[1]/div[1]/div/span[2]/a")
  brandName = brandStep1[0].get_attribute("innerHTML")

  # Get catergory
  categoryStep1 = driver.find_elements(By.XPATH, "//*[@id=\"product-details-control\"]/nav/small/span[3]/a/span")
  categoryName = categoryStep1[0].get_attribute("innerHTML")


  finalDict = {"name": nameFinal, "price": price, "specs": specDict, "images": imageList, "brand": brandName, "category": categoryName}
  
  return finalDict

category = "ram"
with open("ItemLinks/"+category+".text", 'r') as fp1:
  indexToStartAt = int(input("Print the index you want to start the link list at: "))
  ...
  for i in range(indexToStartAt, len(linksList)):
    try:
      ...
      dictList.append(currentItemDict)
      print(dictList)
      with open("ItemInfo/"+category+"/"+category+"_"+str(i)+".json", 'a+') as fp2:
        fp2.write(json.dumps(dictList))
    except:
